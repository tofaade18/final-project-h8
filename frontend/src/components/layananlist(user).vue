      ...
      <div class="d-flex align-items-start justify-content-start p-1 ml-5">
        <div class="rounded-lg bg-secondary p-3 w-100 mb-3">
          <div class="d-flex">
            ...
            <select class="form-select ml-2" v-model="selectedSortOption" @change="sortLayanans">
...
export default {
  ...
  setup() {
    ...
    const selectedSortOption = ref("title");
    ...
    const sortLayanans = () => {
      layananStore.layanans = [...layananStore.layanans];
    };
    const sortedAndFilteredLayanans = computed(() => {
      ...
      // Apply search filter
      if (searchText.value) {
        layanans = layanans.filter(layanan =>
          layanan.title.toLowerCase().includes(searchText.value.toLowerCase())
        );
      }
