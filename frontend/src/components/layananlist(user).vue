...
export default {
  ...
  setup() {
    ...
    const filteredLayanan = computed (() => layananStore.filteredLayanan);
    ...
    const updateSelectedFilter = async () => {
      await layananStore.updateSelectedFilter(selectedFilter.value, searchText.value);
    };
        ...
        <div class="rounded bg-secondary p-3 mb-3" style="min-width: 400px;" >
          <div class="container-fluid d-flex p-0">
            ...
            <select class="form-select ml-2" v-model="selectedSortOption" @change="sortLayanans" style="width: 125px;">
              ...
              <option value="averageRating">Nilai</option>
  ...
  <div class="menu-layanans">
    ...
    <div class="list-group">
      <div class="list-group-item" v-for="(layanan, index) in displayedLayanans" :key="index">
        ...
        <div class="title" style="text-align: center; height: 48px">
          <strong>{{ layanan.title }}</strong>
        </div>
        <div style="display: flex; flex-direction: column; justify-content: flex-start; height: 160px;">
        <div>
          <label><strong>Deskripsi  :</strong></label> {{ layanan.description }}
        </div>
        <div>
          <label><strong>Alamat   :</strong></label> {{ layanan.alamat }}
        </div>
        <div class="rating">
          ...
          <span v-if="layanan.averageRating > 0" class="star-rating" style="color: gold;">
            <span v-for="n in Math.round(layanan.averageRating)" :key="n" class="star" style="font-size: 1.1rem;">★</span>
          </span>
          <span v-else> No reviews yet</span>
        </div>
        <div>
        ...
        <div v-if="currentUser">
          <router-link :to="'/details/' + layanan.id" class="badge badge-primary" style="display: flex; justify-content: center;">Detail</router-link>
        </div>
      </div>
    </div>
    <nav aria-label="Page navigation">
  ...
  setup() {
    ...
    const filteredLayanans = computed(() => {
      ...
      if (selectedSortOption.value === "A-Z") {
        layanans.sort((a, b) => a.title.localeCompare(b.title));
      } else if (selectedSortOption.value === "Z-A") {
        layanans.sort((a, b) => b.title.localeCompare(a.title));
      } else if (selectedSortOption.value === "averageRating") {
        layanans.sort((a, b) => b.averageRating - a.averageRating);
      } else if (selectedSortOption.value === "none") {
        return layanans
      }
      return layanans;
    });
